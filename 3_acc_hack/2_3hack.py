import psycopg2
import requests
from passwords import prob_passwords, test_passwords
...
cur = conn.cursor()
#cur.execute("SELECT DISTINCT full_name FROM student OFFSET 0 LIMIT (SELECT COUNT(DISTINCT full_name)/3 FROM student)")
